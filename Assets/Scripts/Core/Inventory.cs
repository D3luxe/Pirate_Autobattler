...
namespace PirateRoguelike.Services
{
    public class Inventory
    {
        ...
        public bool AddItemAt(ItemInstance item, int index) // Changed return type to bool
        {
        ...
        // NEW: Check if a slot is occupied
        public bool IsSlotOccupied(int index)
        {
            if (index < 0 || index >= MaxSize)
            {
                Debug.LogWarning($"Inventory.IsSlotOccupied: Invalid index {index}");
                return false; // Consider invalid index as not occupied
            }
            return Slots[index].Item != null;
        }

        // NEW: Get the first empty slot index
