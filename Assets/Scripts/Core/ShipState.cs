...
namespace PirateRoguelike.Core
{
    public class ShipState
    {
        ...
        public bool SetEquipment(int index, ItemInstance item) // Changed return type to bool
        {
            if (index < 0 || index >= Equipped.Length)
            {
                Debug.LogWarning($"ShipState.SetEquipment: Invalid index {index}");
                return false;
            }
            if (Equipped[index] != null) // Check if slot is already occupied
            {
                Debug.LogWarning($"ShipState.SetEquipment: Slot {index} is already occupied.");
                return false;
            }
            Equipped[index] = item;
        ...
        public void TakeDamage(int amount)
        {
            ...
            incomingDamage *= (1 - percentDefense);
        ...
        // NEW: Check if an equipment slot is occupied
        public bool IsEquipmentSlotOccupied(int index)
        {
            if (index < 0 || index >= Equipped.Length)
            {
                Debug.LogWarning($"ShipState.IsEquipmentSlotOccupied: Invalid index {index}");
                return false; // Consider invalid index as not occupied
            }
            return Equipped[index] != null;
        }

        // NEW: Get the first empty equipment slot index
        public int GetFirstEmptyEquipmentSlot()
        {
            for (int i = 0; i < Equipped.Length; i++)
            {
                if (Equipped[i] == null)
